...
async def vme_mset_csr_test(dut):
    ...
    inputs_packed = np.array(
        [c["inputs"] for c in cases], dtype=np.uint32).flatten()
    ...
    results = np.frombuffer(raw, dtype=np.uint32).reshape(
        num_cases, RESULT_WORDS)
    ...
    for i, case in enumerate(cases):
        ...
        cocotb.log.info(
            f"[VME case {i}] inputs={case['inputs']}")
    ...
    cocotb.log.info(f"[VME] ✓ All {num_cases} parameterized cases + msetmtypei passed")

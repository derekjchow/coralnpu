...
// -----------------------------------------------------------------------------
// mset* configuration helpers, copied from vme_test_program.cc. The toolchain
// has no Zvt support, so everything is hand-encoded. always_inline keeps the
// named register-variable bindings effective at the call site.
// -----------------------------------------------------------------------------

// msetmtype rs1, rs2 (rd = x0): mtype <- rs1; vtype <- rs2; vl <- 0.
...
// msettn rd, rs1: vl <- min(rs1, vlmax); rd <- vl.

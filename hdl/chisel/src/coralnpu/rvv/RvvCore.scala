...
object GenerateCoreShimSource {
  def apply(instructionLanes: Integer, vlen: Integer, enableVme: Boolean = false): String = {
    ...
    for (i <- 0 until instructionLanes) {
        ...
        if (enableVme) {
          moduleInterface += """    output [31:0] rd_rob2rt_o_GENI_vector_csr_mtype,
            ...
            |    output [13:0] rd_rob2rt_o_GENI_vector_csr_mtm,
            |    output [1:0]  rd_rob2rt_o_GENI_vector_csr_mtk,
            |""".stripMargin.replaceAll("GENI", i.toString)
        }
      ...
      if (enableVme) {
        ...
        coreInstantiation +=
          ("  assign rd_rob2rt_o_GENI_vector_csr_mtype   = 32'd0;\n" +
           ...
           "  assign rd_rob2rt_o_GENI_vector_csr_mtm     = 14'd0;\n" +
           "  assign rd_rob2rt_o_GENI_vector_csr_mtk     = 2'd0;\n"
    ...
    // VME (Zvt) packed mtype CSR view assembled from {mtm, mtk, mtwiden} per
    // spec §15.1.1.2. Tied to 0 when VME_ON is not defined.
    coreInstantiation +=
      "`ifdef VME_ON\n" +
      "  assign configMtype = {8'd0, config_state.mtm, 3'd0, config_state.mtk, 3'd0, config_state.mtwiden};\n" +
      "`else\n" +
...
class RvvCoreShim(p: Parameters) extends Module {
  ...
  if (p.enableVme) {
    ...
    io.configState.bits.mtk.get     := mt(6, 5)
    io.configState.bits.mtm.get     := mt(23, 10)
  }

...
`ifdef VME_ON
        ...
        // selects sub-family.
          ...
          inst_config_state[i+1].mtk     = reg_read_data_i[2*i][6:5];
          inst_config_state[i+1].mtm     = reg_read_data_i[2*i][23:10];
          inst_config_state[i+1].lmul_orig =
              RVVLMUL'(reg_read_data_i[(2*i) + 1][2:0]);
          inst_config_state[i+1].sew =
              RVVSEW'(reg_read_data_i[(2*i) + 1][5:3]);
          ...
          unique case (inst_q[i].bits[15:13])
            ...
            3'b001: begin  // msettm rd, rs1 - mtype.tm <- rs1 (saturated)
              logic [13:0] msettm_new_mtm;
              msettm_new_mtm = (reg_read_data_i[2*i] > 32'h3FFF) ? 14'h3FFF
                                                                  : reg_read_data_i[2*i][13:0];
              inst_config_state[i+1].mtm = msettm_new_mtm;
              mset_writes_rd[i] = 1;
              mset_rd_data[i] = {18'd0, msettm_new_mtm};
            end
            3'b010: begin  // msettk rd, rs1 - mtype.tk <- min(rs1, KMAX). Field
                            // is 2 bits per literal spec, so clamp to 0..3.
              logic [1:0] msettk_new_mtk;
              msettk_new_mtk = (reg_read_data_i[2*i] > 32'd3) ? 2'd3
                                                                : reg_read_data_i[2*i][1:0];
              inst_config_state[i+1].mtk = msettk_new_mtk;
              mset_writes_rd[i] = 1;
              mset_rd_data[i] = {30'd0, msettk_new_mtk};
            end
            3'b011: begin  // msetmtypei - imm[4:0] -> mtype low bits,
              ...
              inst_config_state[i+1].mtk     = 2'b0;
              inst_config_state[i+1].mtm     = 14'd0;
              inst_config_state[i+1].sew     =
...
`ifdef VME_ON
      ...
      config_state_q.mtk     <= 2'b00;
      config_state_q.mtm     <= 14'd0;
`endif
    end else begin
